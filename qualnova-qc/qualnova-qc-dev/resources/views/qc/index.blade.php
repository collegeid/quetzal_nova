...
>
    <div class="flex items-start gap-2">
        ...
        <div>
            ...
            <ul class="list-disc list-inside text-sm leading-relaxed">
                ...
                <li>Tombol <strong>Update Status</strong> hanya untuk Verifikator dalam memvalidasi data.</li>
                <li>Tombol <strong>Preview</strong> digunakan untuk melihat data yang sudah terverifikasi.</li>
                <li>Tombol <strong>Hapus</strong> hanya tersedia untuk Admin dan Super Admin.</li>
            </ul>
            <div class="mt-3 border-t border-blue-200 pt-2">
                ...
                <ul class="list-disc list-inside text-sm leading-relaxed">
                    ...
                    @if($role === 'qc')
                        <li><strong>QC:</strong> Dapat menambah dan mengedit data cacat sebelum diverifikasi.</li>
                        <li>Tidak dapat memverifikasi atau menghapus data.</li>
                    @elseif($role === 'Verifikator')
                        <li><strong>Verifikator:</strong> Hanya dapat mengubah status verifikasi data.</li>
                        <li>Tidak dapat menambah atau menghapus data.</li>
                    @elseif($role === 'admin')
                        <li><strong>Admin:</strong> Dapat menambah, mengedit, menghapus, dan memverifikasi data.</li>
    ...
    @else
        @if(in_array($role, ['qc', 'admin', 'super_admin']))
            ...
            <div class="space-y-3">
                ...
                <div>
                    <input type="file" name="foto_bukti" id="edit_foto_bukti" class="w-full border rounded p-2" accept="image/*"
                           onchange="previewImage(event, 'editPreview')" {{ $role === 'Verifikator' ? 'disabled' : '' }}>
                    <img id="editPreview" src="#" alt="Preview" class="hidden mt-2 rounded border max-h-40 mx-auto">
                </div>

                {{-- Untuk Verifikator, Admin, dan Super Admin, tampilkan dropdown status --}}
                            @if(in_array($role, ['Verifikator', 'admin', 'super_admin']))
                ...
                <div class="flex justify-end gap-2 pt-2">
                    ...
                    <button type="submit"
                            class="{{ $role === 'Verifikator' ? 'bg-green-600' : 'bg-yellow-600' }} text-white px-3 py-1 rounded">
                        {{ $role === 'Verifikator' ? 'Update Status' : 'Update' }}
                    </button>
                </div>
            </div>
        </form>
...
function openEditModal(data) {
    ...
    inputs.forEach(id => {
        ...
        if (role === 'qc' && verified) el.disabled = true;
        if (role === 'Verifikator') el.disabled = true; // Verifikator hanya update status
        if (['admin','super_admin'].includes(role) && verified) el.disabled = true;
    });
