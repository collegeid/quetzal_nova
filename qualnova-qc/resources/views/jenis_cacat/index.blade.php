    ...
    <div class="py-12 bg-[#f8fafc] min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div id="pageGuide" x-data="{ show: true }" x-show="show" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform -translate-y-4" x-transition:enter-end="opacity-100 transform translate-y-0" class="mb-6 bg-blue-50 border border-blue-200 text-blue-800 px-6 py-4 rounded-2xl shadow-sm relative">
                ...
                <button @click="show = false; localStorage.setItem('hideGuide', true)" class="absolute top-4 right-4 text-blue-400 hover:text-blue-600 transition-colors">
            ...
            <div class="bg-white shadow-[0_20px_50px_rgba(0,0,0,0.05)] rounded-[2rem] overflow-hidden border border-gray-100" 
                ...
                <div class="px-10 py-8 border-b border-gray-50 flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <button @click="openCreate = true; namaJenis = ''" class="group relative inline-flex items-center px-8 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-black rounded-2xl transition-all shadow-xl shadow-indigo-200 hover:-translate-y-1 active:translate-y-0 overflow-hidden">
                                ...
                                <tr class="bg-white hover:bg-indigo-50/50 transition-all duration-300 shadow-sm hover:shadow-md transform hover:-translate-y-0.5">
                                    ...
                                    <td class="px-6 py-5 text-right last:rounded-r-2xl">
                                        <div class="flex justify-end gap-2">
                                            <button @click="selected = {{ $jenis->id_jenis }}; namaJenis = '{{ $jenis->nama_jenis }}'; openEdit = true" class="p-2.5 text-amber-500 bg-amber-50 hover:bg-amber-500 hover:text-white rounded-xl transition-all duration-300">
                                            ...
                                            <form id="deleteForm-{{ $jenis->id_jenis }}" action="{{ route('jenis_cacat.destroy', $jenis->id_jenis) }}" method="POST" class="inline">
                                                @csrf @method('DELETE')
                                                <button type="button" onclick="confirmDelete({{ $jenis->id_jenis }})" class="p-2.5 text-rose-500 bg-rose-50 hover:bg-rose-500 hover:text-white rounded-xl transition-all duration-300">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div x-show="openCreate || openEdit" x-cloak class="fixed inset-0 flex items-center justify-center bg-gray-900/40 backdrop-blur-[8px] z-[100]" x-transition.opacity>
                    <div @click.away="openCreate = false; openEdit = false" class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden transform transition-all p-10 border border-gray-100">
                        <div class="mb-8">
                        ...
                        <form :action="openCreate ? '{{ route('jenis_cacat.store') }}' : `/jenis_cacat/${selected}`" method="POST">
                            ...
                            <div class="mb-8">
                                ...
                                <input type="text" name="nama_jenis" x-model="namaJenis" class="w-full bg-gray-50 border-none rounded-2xl focus:ring-4 focus:ring-indigo-500/10 transition-all px-6 py-4 font-bold text-gray-700 placeholder-gray-300" placeholder="Misal: Serat Kain Putus" required>
                            </div>

                            <div class="flex gap-4">
                                <button type="button" @click="openCreate = false; openEdit = false" class="flex-1 px-6 py-4 text-xs font-black text-gray-400 hover:text-gray-600 transition uppercase tracking-widest">
                                    Batal
                                </button>
                                <button type="submit" :class="openCreate ? 'bg-indigo-600 shadow-indigo-100 hover:bg-indigo-700' : 'bg-amber-500 shadow-amber-100 hover:bg-amber-600'" class="flex-[2] px-6 py-4 text-white text-xs font-black rounded-2xl shadow-lg transition-all uppercase tracking-[0.2em]">
                                    Simpan Perubahan
                                </button>
                            </div>
                        </form>
